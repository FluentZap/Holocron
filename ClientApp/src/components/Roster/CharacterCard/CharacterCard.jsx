...
export const TextBox = ({ text, area, delay, className }) => {
    if (!className) className = '';
    return <div className={'font-small animate-fade-in z-5 m2 p2 flex-left data-panel gray-flat scanlines-back roster-text ' + className}
        style={{ gridArea: `${area[0]} / ${area[1]} / span ${area[2]} / span ${area[3]}`, animationDelay: delay }}>
        {text}</div>
}
    ...
    return (
        <div className='animate-fade-in roster-character-card' style={{ gridArea: '4 / 1 / span 10 / span 20', animationDelay: fadeDelay }}>
            ...
            <TextBox area={[6, 15, 4, 5]} delay={boxFade()}
                text={stats.specializations.map(spec => <div key={uuid.v4()}>{spec}</div>)} />
